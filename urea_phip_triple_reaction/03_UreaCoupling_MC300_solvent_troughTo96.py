...
def add_solvent(reaction_condition, solvent):
    ...
    volume_solvent_heading = "Additional solvent to add (uL)"
    ...
    for index, value in enumerate(solvent_df[cpd_id_heading].tolist()):
        ...
        if value == solvent:
            solvent_location = solvent_df[location_heading].tolist()[index]
    for index, value in enumerate(reaction_conditions_df[reaction_heading].tolist()):
        if value == "Urea_sequence":
            ...
            solvent_volume = float(reaction_conditions_df[volume_solvent_heading].tolist()[index])
    ...
    #                              [x.top() for x in reaction_rack.rows(0, to=number_rows)])
    p300_multi.distribute(solvent_volume, source_trough12row.wells(solvent_location),
                          [x.top() for x in reaction_rack.rows(0, to=number_rows)])
    robot.home()

add_solvent(reaction_conditions_df, solvent_df)